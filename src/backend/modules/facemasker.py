...
class FaceMasker:
        ...
        for name in self.mask_colors:
            for dir_ in glob.iglob(str(paths.MASKS_DIR / "*.png")):
                if name == dir_.split("/")[-1].strip(".png"):
                    ...
                    # rotate mask
                    angle = np.arctan2(
                        chin_bottom_point[1] - nose_point[1],
                    ...
                    box_x = (
                        center_x
                        + int(offset * np.cos(radian))
                        - rotated_mask_img.width // 2
                    )
                    box_y = (
                        center_y
                        + int(offset * np.sin(radian))
                    ...
                    tmp.save(
                        self.dst_path
                        + "/"
                        + self.src_path.split("/")[-1].strip(".jpg")
                        + "__"
                        + name
                        + "-mask"
                        + "__"
    ...
    def get_distance_from_point_to_line(point, line_point1, line_point2):
        distance = np.abs(
            (line_point2[1] - line_point1[1]) * point[0]
            + (line_point1[0] - line_point2[0]) * point[1]
            + (line_point2[0] - line_point1[0]) * line_point1[1]
            + (line_point1[1] - line_point2[1]) * line_point1[0]
        ) / np.sqrt(
            (line_point2[1] - line_point1[1]) * (line_point2[1] - line_point1[1])
